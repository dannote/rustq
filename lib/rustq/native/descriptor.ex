defmodule RustQ.Native.Descriptor do
  @moduledoc """
  ...
  @spec resolve!(Index.t(), Ref.t()) :: t()
  def resolve!(%Index{} = index, %Ref{} = ref) do
  ...
  @spec resolve!(Index.t(), Ref.t(), keyword()) :: t()
  def resolve!(%Index{} = index, %Ref{} = ref, opts) when is_list(opts) do
    index
    |> resolve!(ref)
    |> assert_shape!(opts)
  end
  ...
  def assert_shape!(%__MODULE__{} = descriptor, opts) when is_list(opts) do
    ...
    unless return_matches?(method.returns_ast, expected_returns) do
      raise "unexpected native return for #{Ref.format(descriptor.ref)}: #{inspect(method.returns_ast)}"
    end
  ...
  defp assert_args_match!(%__MODULE__{} = descriptor, expected_args) do
    ...
    unless length(actual) == length(expected_args) and
      ...
      raise "unexpected native args for #{Ref.format(descriptor.ref)}: #{inspect(descriptor.method.args)}"
    end
  end

  defp validate_package!(%Index{package: nil}, _ref), do: :ok
  defp validate_package!(_index, %Ref{package: nil}), do: :ok

  defp validate_package!(%Index{package: %{name: name}}, %Ref{package: name}),
    do: :ok

  defp validate_package!(%Index{package: package}, %Ref{} = ref) do
    raise ArgumentError,
